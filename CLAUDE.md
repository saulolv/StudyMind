...
`content-service` is implemented. `api-gateway-auth` and `chat-llm-service` are still scaffolding — nothing but their `@SpringBootApplication` class and a `contextLoads` test. The Python workers (`processing-worker`, `embedding-worker`, `transcription-worker`) and the Angular frontend shown in the README do not exist yet.

`docs/` is referenced throughout `README.md` but is **gitignored** (`/docs/`) and not part of the repo — do not assume those files are readable.
...
Use `./mvnw` instead of `.\mvnw.cmd` from the Bash tool. There is no linter or formatter configured.
...
Infrastructure (Postgres x3, Redis, RabbitMQ, Qdrant, MinIO + bucket init):
...
- The envelope JSON is assembled as an explicit `Map`, not by reflecting over Java field names, because Python workers consume it and the wire contract must not move when a record component is renamed.
...
Still missing and worth adding: failure events (`ProcessingFailed`, `TranscriptionFailed`, `IndexingFailed`) and DLQ/retry conventions. The README sells idempotency, retries and DLQ as a differentiator and no contract covers them yet.
